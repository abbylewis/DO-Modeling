...
TO DO
- Deal with the connection between lines
...
server <- function(input, output, session) {
  ...
  observeEvent(input$click, handlerExpr = {
    ...
    if(!draw()){
      if(input$click$x > max(obs_now$datetime)-1 & input$click$x < max(obs_now$datetime)+1){
        draw(T)
  ...
  observeEvent(input$submit, handlerExpr = {
    ...
    x_raw <- as.Date(vals$x[!is.na(vals$x)])
    y_raw <- vals$y[!is.na(vals$x)]
    x <- as.Date(seq(input$date,input$date+14,by = 1))
  ...
  output$plot= renderPlot({
    plot(start[1],-10, ylab="Oxygen concentration (mg/L)", xlab="", type="p", xlim = c(min(start[year(start)==input$year]), max(stop[year(stop)==input$year])), ylim = c(0,20))
  ...
  output$driver = renderPlot({
    ...
    plot(start[1],-10, xlim = c(min(start[year(start)==input$year]), max(stop[year(stop)==input$year])), ylim = c(0,0.7), xlab = "",xaxt = "n", ylab = "Oxygenation (scfm)")
    rect(xleft = max(obs_now$datetime), xright = (input$date+14), ybot = -1, ytop = 1, col = "lightgrey", alpha = .5, border = NA)
    ...
    plot(start[1],-10, xlim = c(min(start[year(start)==input$year]), max(stop[year(stop)==input$year])), ylim = c(0,20), ylab = "Temp. (ºC)", xlab = "")
