...
server <- function(input, output, session) {
  ...
  output$plot= renderPlot({
    plot(x=obs_now$datetime, y=obs_now$O2_mgL, pch = 19, cex = 1, ylab="Oxygen concentration (mg/L)", xlab="", type="p", xlim = c(start[year(start)==input$year], min(as.Date(input$date, origin = "1970-01-01")+15,max(stop[year(stop)==input$year]))), ylim = c(0,20))
    rect(xleft = max(obs_now$datetime), xright = min(input$date+14, max(stop[year(stop)==input$year])), ybot = -5, ytop = 25, col = "lightgray", alpha = .5, border = NA)
    if(input$model){
      ...
      archived$dates <- seq(start[year(start)==input$year], min(as.Date(input$date, origin = "1970-01-01")+14,max(stop[year(stop)==input$year])),by = 1)
      for(i in 2:101){
  ...
  output$driver = renderPlot({
    ...
    plot(start[1],-10, xlim = c(start[year(start)==input$year], min(as.Date(input$date, origin = "1970-01-01")+15,max(stop[year(stop)==input$year]))), ylim = c(0,0.7), xlab = "",xaxt = "n", ylab = "Oxygenation (scfm)")
