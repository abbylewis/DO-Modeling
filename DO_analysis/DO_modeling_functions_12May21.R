...
run_do_hindcast <- function(inputs, obs, today, n_days = 14, model_name = "normal", uncert = "all", parms, start, stop, realDrivers){
  ...
  if(model_name %in% c("temp","SSS")|realDrivers == T){
    #Run EnKF
    est_out = predict_no_enkf(start = start,
...
#' Function to create predictions without using EnKF
#' 
#' @param start start of summer
#' @param stop end date for this year
#' @param obs observation dataframe
#' @param model_inputs formatted inputs for this year
