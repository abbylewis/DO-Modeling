...
- Added CRPS
- Formalize parameter selection procedure DONE
...
realDrivers = F
=======
#parms = c(0.500000, 1.097871, 0.700000, 3.502894)
...
test_2013 <- run_do_hindcast(inputs_2013,obs, stop, parms = parms, start=start, stop=stop, realDrivers = realDrivers)
lm1 <- lm(inputs_2013$Temp~inputs_2013$Date)
no_csss <- test_2013$Y[2,,] * test_2013$Y[3,,]^(test_2013$drivers[,2,]-20)*test_2013$Y[1,,]/(test_2013$Y[1,,]+test_2013$Y[4,,])
demand <- no_csss
temp <- test_2013$Y[3,,]^(test_2013$drivers[,2,]-20)
o2 <- test_2013$Y[1,,]/(test_2013$Y[1,,]+test_2013$Y[4,,])
r20 <- test_2013$Y[2,,]
for(i in 1:nrow(no_csss)){
  for(j in 1:ncol(no_csss)){
    if(test_2013$drivers[i,1,j] > 0){
      demand[i,j] = demand[i,j]*test_2013$Y[5,i,j]
    }
  }
}
demand_all = data_frame(no_csss_mean = apply(no_csss, 1, mean), no_csss_sd = apply(no_csss, 1, sd), demand_mean = apply(demand, 1, mean), demand_sd = apply(demand, 1, sd), temp_mean = apply(temp, 1, mean), temp_sd = apply(temp, 1, sd), o2_mean = apply(o2,1,mean),o2_sd = apply(o2,1,sd),r20_mean = apply(r20,1,mean), r20_sd = apply(r20,1,sd),dsim = 1:nrow(no_csss))
...
test_2014 <- run_do_hindcast(inputs_2014,obs,stop, parms = parms, start=start, stop=stop, realDrivers = realDrivers)
...
no_csss <- test_2014$Y[2,,] * test_2014$Y[3,,]^(test_2014$drivers[,2,]-20)*test_2014$Y[1,,]/(test_2014$Y[1,,]+test_2014$Y[4,,])
demand <- no_csss
temp <- test_2014$Y[3,,]^(test_2014$drivers[,2,]-20)
o2 <- test_2014$Y[1,,]/(test_2014$Y[1,,]+test_2014$Y[4,,])
r20 <- test_2014$Y[2,,]
for(i in 1:nrow(no_csss)){
  for(j in 1:ncol(no_csss)){
    if(test_2014$drivers[i,1,j] > 0){
      demand[i,j] = demand[i,j]*test_2014$Y[5,i,j]
    }
  }
}
demand_all_2014 = data_frame(no_csss_mean = apply(no_csss, 1, mean), no_csss_sd = apply(no_csss, 1, sd), demand_mean = apply(demand, 1, mean), demand_sd = apply(demand, 1, sd), temp_mean = apply(temp, 1, mean), temp_sd = apply(temp, 1, sd), o2_mean = apply(o2,1,mean),o2_sd = apply(o2,1,sd),r20_mean = apply(r20,1,mean), r20_sd = apply(r20,1,sd),dsim = 1:nrow(no_csss))
...
test_2015 <- run_do_hindcast(inputs_2015,obs,stop, parms = parms, start=start, stop=stop, realDrivers = realDrivers)
lm3 <- lm(inputs_2015$Temp~inputs_2015$Date)
no_csss <- test_2015$Y[2,,] * test_2015$Y[3,,]^(test_2015$drivers[,2,]-20)*test_2015$Y[1,,]/(test_2015$Y[1,,]+test_2015$Y[4,,])
demand <- no_csss
temp <- test_2015$Y[3,,]^(test_2015$drivers[,2,]-20)
o2 <- test_2015$Y[1,,]/(test_2015$Y[1,,]+test_2015$Y[4,,])
r20 <- test_2015$Y[2,,]
for(i in 1:nrow(no_csss)){
  for(j in 1:ncol(no_csss)){
    if(test_2015$drivers[i,1,j] > 0){
      demand[i,j] = demand[i,j]*test_2015$Y[5,i,j]
    }
  }
}
demand_all_2015 = data_frame(no_csss_mean = apply(no_csss, 1, mean), no_csss_sd = apply(no_csss, 1, sd), demand_mean = apply(demand, 1, mean), demand_sd = apply(demand, 1, sd), temp_mean = apply(temp, 1, mean), temp_sd = apply(temp, 1, sd), o2_mean = apply(o2,1,mean),o2_sd = apply(o2,1,sd),r20_mean = apply(r20,1,mean), r20_sd = apply(r20,1,sd),dsim = 1:nrow(no_csss))
...
test_2016 <- run_do_hindcast(inputs_2016,obs,stop, parms = parms, start=start, stop=stop, realDrivers = realDrivers)
lm4 <- lm(inputs_2016$Temp~inputs_2016$Date)
no_csss <- test_2016$Y[2,,] * test_2016$Y[3,,]^(test_2016$drivers[,2,]-20)*test_2016$Y[1,,]/(test_2016$Y[1,,]+test_2016$Y[4,,])
demand <- no_csss
temp <- test_2016$Y[3,,]^(test_2016$drivers[,2,]-20)
o2 <- test_2016$Y[1,,]/(test_2016$Y[1,,]+test_2016$Y[4,,])
r20 <- test_2016$Y[2,,]
for(i in 1:nrow(no_csss)){
  for(j in 1:ncol(no_csss)){
    if(test_2016$drivers[i,1,j] > 0){
      demand[i,j] = demand[i,j]*test_2016$Y[5,i,j]
    }
  }
}
demand_all_2016 = data_frame(no_csss_mean = apply(no_csss, 1, mean), no_csss_sd = apply(no_csss, 1, sd), demand_mean = apply(demand, 1, mean), demand_sd = apply(demand, 1, sd), temp_mean = apply(temp, 1, mean), temp_sd = apply(temp, 1, sd), o2_mean = apply(o2,1,mean),o2_sd = apply(o2,1,sd),r20_mean = apply(r20,1,mean), r20_sd = apply(r20,1,sd),dsim = 1:nrow(no_csss))
...
inputs_2018 <- inputs_year(start,stop,CTD,SSS)
test_2018 <- run_do_hindcast(inputs_2018,obs,stop, parms = parms, start=start, stop=stop, realDrivers = realDrivers)
no_csss <- test_2018$Y[2,,] * test_2018$Y[3,,]^(test_2018$drivers[,2,]-20)*test_2018$Y[1,,]/(test_2018$Y[1,,]+test_2018$Y[4,,])
demand <- no_csss
temp <- test_2018$Y[3,,]^(test_2018$drivers[,2,]-20)
o2 <- test_2018$Y[1,,]/(test_2018$Y[1,,]+test_2018$Y[4,,])
r20 <- test_2018$Y[2,,]
for(i in 1:nrow(no_csss)){
  for(j in 1:ncol(no_csss)){
    if(test_2018$drivers[i,1,j] > 0){
      demand[i,j] = demand[i,j]*test_2018$Y[5,i,j]
    }
  }
}
demand_all_2018 = data_frame(no_csss_mean = apply(no_csss, 1, mean), no_csss_sd = apply(no_csss, 1, sd), demand_mean = apply(demand, 1, mean), demand_sd = apply(demand, 1, sd), temp_mean = apply(temp, 1, mean), temp_sd = apply(temp, 1, sd), o2_mean = apply(o2,1,mean),o2_sd = apply(o2,1,sd),r20_mean = apply(r20,1,mean), r20_sd = apply(r20,1,sd),dsim = 1:nrow(no_csss))
...
inputs_2019 <- inputs_year(start,stop,CTD,SSS)
test_2019 <- run_do_hindcast(inputs_2019,obs,stop, parms = parms, start=start, stop=stop, realDrivers = realDrivers)
no_csss <- test_2019$Y[2,,] * test_2019$Y[3,,]^(test_2019$drivers[,2,]-20)*test_2019$Y[1,,]/(test_2019$Y[1,,]+test_2019$Y[4,,])
demand <- no_csss
temp <- test_2019$Y[3,,]^(test_2019$drivers[,2,]-20)
o2 <- test_2019$Y[1,,]/(test_2019$Y[1,,]+test_2019$Y[4,,])
r20 <- test_2019$Y[2,,]
for(i in 1:nrow(no_csss)){
  for(j in 1:ncol(no_csss)){
    if(test_2019$drivers[i,1,j] > 0){
      demand[i,j] = demand[i,j]*test_2019$Y[5,i,j]
    }
  }
}
demand_all_2019 = data_frame(no_csss_mean = apply(no_csss, 1, mean), no_csss_sd = apply(no_csss, 1, sd), demand_mean = apply(demand, 1, mean), demand_sd = apply(demand, 1, sd), temp_mean = apply(temp, 1, mean), temp_sd = apply(temp, 1, sd), o2_mean = apply(o2,1,mean),o2_sd = apply(o2,1,sd),r20_mean = apply(r20,1,mean), r20_sd = apply(r20,1,sd),dsim = 1:nrow(no_csss))
...
demand_all_long = demand_all_final%>%
  pivot_longer(cols = c("no_csss_mean","demand_mean","temp_mean","o2_mean","r20_mean"), names_to = "mean_type", values_to = "mean")%>%
  pivot_longer(cols = c("no_csss_sd","demand_sd","temp_sd","o2_sd","r20_sd"), names_to = "sd_type", values_to = "sd")%>%
  filter(gsub("_mean","", mean_type) == gsub("_sd","",sd_type))%>%
  mutate(type = gsub("_mean","", mean_type))%>%
  dplyr::select(-mean_type,-sd_type)
...
```{r}
<<<<<<< HEAD
for(model_name in c("full", "temp","o2")){ #,"temp","o2","sss"
...
#for(model_name in c("full","temp","o2")){ 
  #for(uncert in c("param","all","init","proc","driver")){ 
    ...
    parms = c(0.5108, 1.1011, 0.6954, 3.6687)
    #parms = c(R20=.7, theta=1.12, ko2 = .5, sss_scalar = 3)
    #parms <- c(
    #  R20 = mean(.65, .7, .75, .65),
...
=======
    ...
    rmse_2016_filt <- createRmseDF_filt(n_days, results_2016, obs, cali = 14/run_space)
    rmse_2016 <- createRmseDF(n_days, results_2016, cali = 14/run_space)
    calcRmseTotal(n_days, results_2016, cali = 14/run_space)
    rmse_2016
    rmse_2016_filt
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
...
rmse_persist_2013 <- createRmseDF(n_days, persist_2013, cali = 14/run_space)

#2014
start = start_all[year(start_all)==2014]
stop = stop_all[year(stop_all)==2014]
persist_2014 <- persistenceForecast(start, stop, n_days, run_space, obs)
rmse_persist_2014 <- createRmseDF(n_days, persist_2014, cali = 14/run_space)

#2015
start = start_all[year(start_all)==2015]
stop = stop_all[year(stop_all)==2015]
persist_2015 <- persistenceForecast(start, stop, n_days, run_space, obs)
rmse_persist_2015 <- createRmseDF(n_days, persist_2015, cali = 14/run_space)

#2016
start = start_all[year(start_all)==2016]
stop = stop_all[year(stop_all)==2016]
persist_2016 <- persistenceForecast(start, stop, n_days, run_space, obs)
rmse_persist_2016 <- createRmseDF(n_days, persist_2016, cali = 14/run_space)

#2017
start = start_all[year(start_all)==2017]
stop = stop_all[year(stop_all)==2017]
persist_2017 <- persistenceForecast(start, stop, n_days, run_space, obs)
rmse_persist_2017 <- createRmseDF(n_days, persist_2017, cali = 14/run_space)

#2018
start = start_all[year(start_all)==2018]
stop = stop_all[year(stop_all)==2018]
persist_2018 <- persistenceForecast(start, stop, n_days, run_space, obs)
rmse_persist_2018 <- createRmseDF(n_days, persist_2018, cali = 14/run_space)

#2019
start = start_all[year(start_all)==2019]
stop = stop_all[year(stop_all)==2019]
persist_2019 <- persistenceForecast(start, stop, n_days, run_space, obs)
rmse_persist_2019 <- createRmseDF(n_days, persist_2019, cali = 14/run_space)
...
<<<<<<< HEAD
png("../DO_modeling_figures/15Sep20/uncert_fract7.png",width = 6, height = 4, units = "in", res = 300)
=======
png("../DO_modeling_figures/14Jul20/uncert_fract7.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
all_sd_std %>%
  filter(uncert != "all")%>%
...
<<<<<<< HEAD
png("../DO_modeling_figures/15Sep20/uncert_fract14.png",width = 6, height = 4, units = "in", res = 300)
=======
png("../DO_modeling_figures/14Jul20/uncert_fract14.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
all_sd_std %>%
  filter(uncert != "all")%>%
...
<<<<<<< HEAD
png("../DO_modeling_figures/15Sep20/uncert_season7.png",width = 6, height = 4, units = "in", res = 300)
=======
png("../DO_modeling_figures/14Jul20/uncert_season7.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
all_sd %>%
  filter(uncert != "all")%>%
...
<<<<<<< HEAD
png("../DO_modeling_figures/15Sep20/uncert_season14.png",width = 6, height = 4, units = "in", res = 300)
=======
png("../DO_modeling_figures/14Jul20/uncert_season14.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
all_sd %>%
...
<<<<<<< HEAD
png("../DO_modeling_figures/15Sep20/uncert_horiz.png",width = 6, height = 4, units = "in", res = 300)
=======
png("../DO_modeling_figures/14Jul20/uncert_horiz.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
SDs_final%>%
  filter(uncert != "all")%>%
...
for(year in seq(2013,2019)){
  ...
  o2 <- read.csv(paste("../DO_modeling_results/14Jul20/forecast_",year,"_o2_model_all_uncert.csv",sep = ""))%>%
    mutate(model = "o2",
           year = year)
  #sss <- read.csv(paste("../DO_modeling_results/14Jul20/forecast_",year,"_sss_model_all_uncert.csv",sep = ""))%>%
  #  mutate(model = "sss",
  #         year = year)
...
=======
png("../DO_modeling_figures/14Jul20/bias_horizon.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
output_df%>%
...
#crps
<<<<<<< HEAD
png("../DO_modeling_figures/15Sep20/crps_season7.png",width = 6, height = 4, units = "in", res = 300)
=======
png("../DO_modeling_figures/14Jul20/crps_season7.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
all_model %>%
...
<<<<<<< HEAD
png("../DO_modeling_figures/15Sep20/crps_season14.png",width = 6, height = 4, units = "in", res = 300)
=======
png("../DO_modeling_figures/14Jul20/crps_season14.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
all_model %>%
...
<<<<<<< HEAD
png("../DO_modeling_figures/15Sep20/crps_horiz.png",width = 6, height = 4, units = "in", res = 300)
=======
png("../DO_modeling_figures/14Jul20/crps_horiz.png",width = 6, height = 4, units = "in", res = 300)
>>>>>>> 6e2ed9a39c1cde661e6e47fba1a0992f7f1316cf
crps_final%>%
