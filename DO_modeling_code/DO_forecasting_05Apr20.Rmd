...
Run forecasts!
```{r}
for(uncert in c("init","driver","all")){ #add "param" back here!
  ...
  gif = F
  #uncert = "param" #Options = "all","init","driver","param"
...
all <- read.csv(paste("../DO_modeling_results/06Apr20/forecast_2013_full_model_all_uncert.csv",sep = ""))
...
for(year in seq(2013,2019)){
  full <- read.csv(paste("../DO_modeling_results/06Apr20/forecast_",year,"_full_model_all_uncert.csv",sep = ""))%>%
    mutate(uncert = "all",
           year = year)
  full_sd <- sd_sum(full,"all",year)
  param <- read.csv(paste("../DO_modeling_results/06Apr20/forecast_",year,"_full_model_param_uncert.csv",sep = ""))%>%
    mutate(uncert = "param",
           year = year)
  param_sd <- sd_sum(param,"param",year)
  driver <- read.csv(paste("../DO_modeling_results/06Apr20/forecast_",year,"_full_model_driver_uncert.csv",sep = ""))%>%
    mutate(uncert = "driver",
           year = year)
  driver_sd <- sd_sum(driver,"driver",year)
  init <- read.csv(paste("../DO_modeling_results/06Apr20/forecast_",year,"_full_model_init_uncert.csv",sep = ""))%>%
    mutate(uncert = "init",
           year = year)
...
all_sd %>%
  ...
  geom_line(aes(x = as.Date(TODAY), y = PLUS7_sd), data = all_sd %>% filter(uncert == "all"))
...
all_sd %>%
  ...
  geom_bar(stat = "identity", width = 1)+
  facet_wrap("year", scales = "free_x")+
  scale_fill_discrete(name = "Uncertainty", limits = c("driver","init","param"), 
                      labels = c("driver","initial cond","parameter"))+
  xlab("")+
  ylab("Variance (14 day horizon)")+
  geom_line(aes(x = as.Date(TODAY), y = PLUS14_sd, lty=uncert), data = all_sd %>% filter(uncert == "all", year !=2017))+
