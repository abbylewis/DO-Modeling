...
source('DO_forecasting_functions_20Jan21.R')
...
To do:
- Make it so temp and o2 models don't use the mean they just don't include the other variable at all
- Set up real drivers for temp and o2 models
...
toCalcProcUncert <- all %>%
  filter(year(as.Date(TODAY))<=2017,
         uncert == "all")%>%
  left_join(all%>%
              dplyr::select(TODAY, PLUS1_obs)%>%
              mutate(TODAY = TODAY+1,
                     PLUS1_obs = !is.na(PLUS1_obs))%>%
              rename(obs_yn = PLUS1_obs))%>%
  filter(obs_yn)
n <- seq(1,n_days)
