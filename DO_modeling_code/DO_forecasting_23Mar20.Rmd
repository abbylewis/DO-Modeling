...
runForecasts <- function(start, stop, n_days, run_space, obs, gif = TRUE, archiveForecasts = FALSE, remove = FALSE, delay = 30, model_name = "full"){
  ...
  if(model_name == "temp"){inputs_thisYear$Conc <- avg_o2}
  if(model_name == "o2"){inputs_thisYear$Temp <- avg_temp}
  today <- start
  if(gif == TRUE){
...
results_2014 <- runForecasts(start, stop, n_days, run_space, obs, delay = 5, model_name = "temp")
