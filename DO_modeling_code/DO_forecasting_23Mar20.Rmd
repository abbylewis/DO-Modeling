...
To do:
- Parameter uncertainty is wildly wrong
- Observation uncertainty is not right either (== 0)
...
- Finished driver, init cond, and obs uncertainty!
...
SDs <- read.csv(paste("../DO_modeling_results/31Mar20/forecast_2013_full_model_all_uncert.csv",sep = ""))
...
for(year in seq(2013,2019)){
  ...
  full_sd <- full%>%
    filter(!is.na(TODAY))%>%
    select(seq(31,44))%>%
    replace_na(list(rep(0,ncol(full))))%>%
    summarize_all(~mean(.))%>%
    mutate(uncert = "all",
           year = year)
  obs_1 <- read.csv(paste("../DO_modeling_results/31Mar20/forecast_",year,"_full_model_obs_uncert.csv",sep = ""))
  obs_sd <- obs_1%>%
    filter(!is.na(TODAY))%>%
    select(seq(31,44))%>%
    replace_na(list(rep(0,ncol(obs_1))))%>%
    summarize_all(~mean(.))%>%
    mutate(uncert = "obs",
           year = year)
  ...
  param_sd <- param%>%
    filter(!is.na(TODAY))%>%
    select(seq(31,44))%>%
    replace_na(list(rep(0,ncol(param))))%>%
    summarize_all(~mean(.))%>%
    mutate(uncert = "param",
           year = year)
  ...
  driver_sd <- driver%>%
    filter(!is.na(TODAY))%>%
    select(seq(31,44))%>%
    replace_na(list(rep(0,ncol(driver))))%>%
    summarize_all(~mean(.))%>%
    mutate(uncert = "driver",
           year = year)
  init <- read.csv(paste("../DO_modeling_results/31Mar20/forecast_",year,"_full_model_init_uncert.csv",sep = ""))
  init_sd <- init%>%
    filter(!is.na(TODAY))%>%
    select(seq(31,44))%>%
    replace_na(list(rep(0,ncol(init))))%>%
    summarize_all(~mean(.))%>%
    mutate(uncert = "init",
           year = year)
  SDs = SDs%>%
