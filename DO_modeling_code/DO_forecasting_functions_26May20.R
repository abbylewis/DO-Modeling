...
run_do_hindcast <- function(inputs, obs, today, n_days = 14, model_name = "normal", uncert = "all"){
  ...
  last_cast_row <- max(which(!is.na(inputs$Temp)))
  inputs$Temp[last_cast_row:nrow(inputs)]<- inputs$Temp[last_cast_row]+temp_slope*seq(last_cast_row,nrow(inputs), by = 1)
  inputs = inputs%>%
    mutate(Conc = na.approx(Conc, rule = 2), #rule = 2 makes points outside the temporal range of observed points equal to the last observation
           Temp = na.approx(Temp, rule = 2),# This shouldn't be doing anything because of the line above
