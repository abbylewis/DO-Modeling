...
param_est_fun <- function(parms){
  ...
  rmse_2013 <- createRmseDF(n_days, results_2013, cali = run_space/14) # Calculate rmse for all days
  ...
  #write.csv(results_year,paste("../DO_modeling_results/",date,"/forecast_2013_",model_name,"_model_",uncert,"_uncert.csv", sep = ""))
  rmse_2014 <- createRmseDF(n_days, results_2014, cali = run_space/14) # Calculate rmse for all days
  ...
  #write.csv(results_year,paste("../DO_modeling_results/",date,"/forecast_2013_",model_name,"_model_",uncert,"_uncert.csv", sep = ""))
  rmse_2015 <- createRmseDF(n_days, results_2015, cali = run_space/14) # Calculate rmse for all days
  ...
  #write.csv(results_year,paste("../DO_modeling_results/",date,"/forecast_2013_",model_name,"_model_",uncert,"_uncert.csv", sep = ""))
  rmse_2016 <- createRmseDF(n_days, results_2016, cali = run_space/14) # Calculate rmse for all days
  
  rmses = c(rmse_2013$val, rmse_2014$val, rmse_2015$val, rmse_2016$val)
...
```{r}
#Specifications
  run_space <- 7 #Number of days between runs of the model
...
par.set = makeNumericParamSet(len = 4L, id = "parms",
                              lower = c(.5,1.02,.1,2), upper = c(1,1.2,.7,4),vector = TRUE) 

objective.fun = makeSingleObjectiveFunction(name = paste(year), 
                                           id = NULL,
                                           description = paste0("enKF DO function for ",year), 
                                           fn = param_est_fun, 
                                           par.set = par.set,
                                           noisy = F, 
                                           fn.mean = NULL,
