...
O2_model <- function(times, states, parms, inputs){
  ...
  #CALCULATIONS
  R = R20*theta^(temp-20)*O2_mgL/(O2_mgL+ko2)*(chla)/(chla+k_chla)*hypo_ratio
  scalar = 1
  if(SSS>0){
    scalar = sss_scalar
  }
...
driver_cv = c(SSS = .1,
    ...
    temp = .1,
    hypo_volume = .2,
    Chla = .2) # Coefficient of variation of driver data 
init_cond_cv = .1
obs_cv = 0.1
```
...
run_do_hindcast <- function(start,stop){
  ...
  model_inputs <- list(datetime = inputs$Date,
    ...
    entrain = rep(0,simulation_time),
    temp = inputs$Temp,
    hypo_volume = inputs$SA/inputs$hypoVolume/median_hypo_ratio,
    chla = c(rep(inputs$Chla[1],14,),head(inputs$Chla,-14)))
  model_inputs<- data.frame(model_inputs)
...
rmse(apply(est2013$Y[1,,], 1, FUN = mean),est2013$obs[1,,])
rmse(apply(est2014$Y[1,,], 1, FUN = mean),est2014$obs[1,,])
rmse(apply(est2015$Y[1,,], 1, FUN = mean),est2015$obs[1,,])
rmse(apply(est2016$Y[1,,], 1, FUN = mean),est2016$obs[1,,])
rmse(apply(est2017$Y[1,,], 1, FUN = mean),est2017$obs[1,,])
rmse(apply(est2018$Y[1,,], 1, FUN = mean),est2018$obs[1,,])
rmse(apply(est2019$Y[1,,], 1, FUN = mean),est2019$obs[1,,])
...
jpeg("../DO_modeling_figures/hindcast_2013_2019_13Dec19.jpg",width = 7.5,height = 4,units = "in", res = 300)
par(mfrow = c(2,4))
plot_o2(est2013)
plot_o2(est2014)
plot_o2(est2015)
...
jpeg("../DO_modeling_figures/hindcast_2014_2019_param5_19Feb20.jpg",width = 7.5,height = 4,units = "in", res = 300)
par(mfrow = c(2,3))
plot_param(est2014,5, param_names[5])
plot_param(est2015,5, param_names[5])
plot_param(est2016,5, param_names[5])
plot_param(est2017,5, param_names[5])
plot_param(est2018,5, param_names[5])
plot_param(est2019,5, param_names[5])
dev.off()
