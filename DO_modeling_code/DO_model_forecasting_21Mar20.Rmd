...
runForecasts <- function(start, stop, n_days, run_space, obs, gif = TRUE, archiveForecasts = FALSE, remove = FALSE, delay = 30){
  ...
  while(today < stop){
    ...
    if(archiveForecasts == TRUE){
      dateRun <- format(Sys.Date(),"%d%b%y")
      year <- year(start)
      dir1 <- paste("../Archived_forecasts",dateRun,sep = "/")
      dir2<-paste(dir1,year,sep = "/")
      dir.create(dir1)
      dir.create(dir2)
      if(gif == TRUE){
  ...
  if (gif == TRUE){
    convert <- paste("convert -delay ",delay," *.png animated_count_down.gif", sep = "")
    system(convert)
    dev.off()
    # Remove png files
    if(remove == TRUE){
...
results_2014 <- runForecasts(start, stop, n_days, run_space, obs, archiveForecasts = TRUE)
