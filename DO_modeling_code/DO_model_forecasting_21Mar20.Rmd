...
- Need a way of quantifying how good the model is
  ...
  - So this would be best with a final spreadsheet where there's a column for predictions a certain number of days out and observations on that day
  - TODAY PLUS1 PLUS1_OBS PLUS2 PLUS2_OBS, etc
...
rows <- floor(days/run_space)
cols <- n_days*2+1
results_2016 <- matrix(NA, nrow = rows, ncol = cols)
n <- rep(seq(1,n_days),each = 2)
type <- rep(c("_pred","_obs"),each = n_days)
col_names <- c("TODAY",paste("PLUS",n,type,sep = ""))
colnames(results_2016)<-col_names

inputs_2016 <- inputs_year(start,stop)
today <- start
dir.create("../DO_modeling_figures/22Mar20/2016")
setwd("../DO_modeling_figures/22Mar20/2016")
png(file="example%02d.png", width=480, height=480)
while(today < stop){
  row<- as.numeric(difftime(today,start))/run_space+1
  est2016 <- run_do_hindcast(inputs_2016, obs, today, n_days)
  mean_o2_est = apply(est2016$Y[1,,], 1, FUN = mean)
  results_2016[row,2:(1+n_days)]<-tail(mean_o2_est, n = n_days)
