...
createRmseDF <- function(n_days,results){
  ...
  val <- rep(NA,n_days)
  class(val) <- "numeric"
  rmse_2016 <- data.frame(n,val)
  results_2016 <- as.data.frame(results_2016)
  for(i in seq(1:n_days)){
    predicted <- results_2016[i+1]
    observed <- results_2016[i+1+n_days]
    rmse_2016$val[rmse_2016$n == i] <- rmse(observed[!is.na(observed)], predicted[!is.na(observed)])
  }
  rmse_2016
}

runForecasts <- function(start, stop, n_days, run_space, obs, gif = TRUE, archiveForecasts = FALSE, remove = FALSE){
  #Create results dataframe
  results<- createResultsDF(start, stop)
  #Create obs file with all dates (filling in NAs)
  ...
  if(gif == TRUE){
    ...
    dir.create(dir2)
    setwd(dir2)
    png(file="forecast%02d.png", width=480, height=480)
  }
  while(today < stop){
...
#Run forecast!
results_2014 <- runForecasts(start, stop, n_days, run_space, obs)

# Calculate rmse for all days
rmse_2014 <- createRmseDF(n_days, results_2014)
```
...
#Run forecast!
results_2016 <- runForecasts(start, stop, n_days, run_space, obs)
...
#Run forecast
results_2017 <- runForecasts(start, stop, n_days, run_space, obs)
...
#Run forecast
results_2018 <- runForecasts(start, stop, n_days, run_space, obs)
