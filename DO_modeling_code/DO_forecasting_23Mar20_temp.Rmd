...
Created 23 March 20. Making a forecast that is not sensitive to oxygen
...
run_do_hindcast <- function(inputs, obs, today, n_days = 14){
  ...
  model_inputs <- list(datetime = inputs$Date,
    ...
    temp = inputs$Temp,
    O2_mgL = rep(mean(inputs$Conc, na.rm = TRUE),nrow(inputs)))
...
runForecasts <- function(start, stop, n_days, run_space, obs, gif = TRUE, archiveForecasts = FALSE, remove = FALSE, delay = 30){
  ...
  if(gif == TRUE){
    ...
    dir2<-paste(dir1,"/",year,"_temp",sep = "")
    dir.create(dir1)
    dir.create(dir2)
    #setwd(dir2)
  ...
  while(today < stop){
    ...
    if(archiveForecasts == TRUE){
      ...
      dir2.1<-paste(dir1.1,"/",year,"_temp",sep = "")
      dir.create(dir1.1)
      dir.create(dir2.1)
