...
inputs = dates %>%
  left_join(CTD %>%
              dplyr::select(Date,Conc,Temp,hypoVolume,thermo_depth,Chla_ugL), by = "Date")%>%
  left_join(SSS %>% 
              dplyr::select(time, scfm), by = c("Date"="time"))%>%
  arrange(Date)
inputs$hypoVolume <- mean(inputs$hypoVolume, na.rm = TRUE)
inputs$Chla<-ifelse(all(is.na(inputs$Chla_ugL)),1,inputs$Chla_ugL)
inputs = inputs%>%
  filter(Date<=stop, 
         Date>=start)%>%
  group_by(Date)%>%
