...
server <- function(input, output, session) {
  ...
  output$preImage <- renderImage({
    ...
    if(model == "_all"){model <- ""}
    filename <- paste('15Apr20/',year,model,'/forecast', date_num, '.png', sep='')
    if(input$gif == T){
      filename <- paste('15Apr20/',year,model,'/animated_forecast.gif',sep="")
    }
  ...
  output$forecast_date <- renderText({
    paste("Forecast generated on 15 April 2020")
  })
  
  
  output$rmse <- renderImage({
    filename <- "./15Apr20/RMSE_all.jpeg"
  ...
  output$uncert_all <- renderImage({
    filename <- "./15Apr20/uncert_horiz.png"
  ...
  output$uncert_year <- renderImage({
    filename <- "./15Apr20/uncert_season7.png"
    height <- session$clientData$output_preImage_height
    list(src = filename,
         height=height,
  ...
  output$param <- renderImage({
    model <- input$model
    if(model == "_all"){model <- ""}
    filename <- paste('15Apr20/',input$year,model,'/params.jpeg', sep='')
    height <- session$clientData$output_preImage_height
    list(src = filename,
         height=height,
